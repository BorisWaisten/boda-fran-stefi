        ...
        {informacion.map((evento, index) => (
          <div key={index} className="flex flex-col items-center ">
            <div className="bg-black rounded-full p-4 w-20 h-20 md:w-24 md:h-24 flex items-center justify-center mb-4 ">
              <img src={evento.icon} alt={evento.title} className="w-10 h-10" />
            </div>
            <h3 className="text-[#FDB913] font-centuryBold text-xl md:text-2xl uppercase">{evento.title}</h3>
            <p className="font-centuryBold w-[25%] md:w-[35%] md:text-xl mt-2">{evento.day} | {evento.time} |</p>

            <p className="font-retro text-[#FDB913] text-3xl md:text-5xl mt-1">
              "{evento.type}
            </p>

            <p className="font-retro text-[#FDB913] text-3xl md:text-5xl mb-1">
              {evento.place}”
            </p>
