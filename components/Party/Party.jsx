'use client';
import React from 'react';
import '../components.css';
import { useRouter } from 'next/navigation';
// import Button from '../ui/buttons'; // Importa el nuevo componente de botón

const Party = () => {
  const router = useRouter();

  const handleSongSuggestionClick = () => {
    // Redirige a la página de sugerencia de canciones
    router.push('/song');
  };

  return (
      <div className="relative container mt-5 py-8 text-center mx-auto">

        <h2 className=" text-lg md:text-3xl lg:text-5xl text-[#FDB913] font-centuryBold mb-8 " >
         !LA FIESTA!
        </h2>
        <h3 className={`font-retro italic text-sm md:text-lg lg:text-4xl mb-8 `} >
          ¡Algunos detalles a tener en cuenta!
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 ">
          {/* Sugerir Canción */}
          <div className='p-4'>
             <div className='flex justify-center'>
              <img className="w-1/3" src="/musica.svg" alt="party" />
            </div>
            <h3 className={`text-3xl font-centuryBold text-[#FDB913]  mb-4 `}>
              MÚSICA
            </h3>
            <div className='w-1/2 mx-auto text-center flex justify-center'>
              <p className={`text-sm sm:text-lg  font-centuryItalic  mb-4 `}>
                ¿Cuál es la canción que no debe faltar en la PlayList de la fiesta?
              </p>
            </div>
            <button
              onClick={handleSongSuggestionClick}
              className="mt-4 bg-black text-[#FDB913] font-centuryBold py-2 px-6 rounded-full shadow-md hover:bg-[#FDB913] transition-colors"
              >
              SUGERIR CANCIÓN
            </button>
          </div>

          {/* Dress Code */}
          <div className=' p-4'>
            <div className='flex justify-center'>
              <img className="w-1/3" src="/vestimenta.svg" alt="party" />
            </div>
            <h3 className={`text-3xl font-centuryBold text-[#FDB913]  mb-4 `}>
              VESTIMENTA
            </h3>
            <p className={`text-sm sm:text-lg font-centuryItalic  pt-4 `}>
              Formal - Elegante
            </p>
            <button
              className="mt-4 bg-black text-[#FDB913] font-centuryBold py-2 px-6 rounded-full shadow-md hover:bg-[#FDB913] transition-colors"
            onClick={() => router.push('/outfits')}
            >
              INSPIRATE
            </button>
          </div>
        </div>
      </div>
  );
};

export default Party;
